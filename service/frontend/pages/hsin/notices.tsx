...
const Notices: NextPage = () => {
  ...
  const paramsObj = {
    ...
    ...(!query.searchType
      ? {
          ...
          ['_where[_or][2][author_contains]']: query.query as string,
        }
      : {
  ...
  const { data: board, error: boardError } = useSWR(`/board/notices?${params}`);
  const { data: notices, error: noticesError } = useSWR(
    `/board/notices?notice=true`
  );
  const { data: total, error: totalError } = useSWR(
    `/board/notices/count?${params}`
  );
  ...
  >(() => [...(notices ?? []), ...(board ?? [])], [notices, board, size], []);
      ...
      <ContentSection className={`font-notosans`}>
        <Content>
          ...
          <Column className="board">
            ...
            {list.map(({ id, title, created_at, notice, author, views }, i) => {
              return (
                <Link
                  href={`/hsin/notices/${id}${globalThis.location?.search}`}
                  key={`board-item-${i}`}
                >
                  <Row className={'row ' + (notice ? 'notice' : '')}>
                    <div className="no">{notice ? '공지' : id}</div>
                    <div className="title">{title}</div>
                    <div className="author">{author ?? '건강기능식품협회'}</div>
