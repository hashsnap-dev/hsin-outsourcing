    ...
    return next.handle().pipe(
      map(async (returnedValue: FindResult<any>) => {
        ...
        if (returnedValue.type === 'item') {
          ...
          const { _id, ...cData } = result;
          return {
            type,
            data: cData,
          };
        } else if (returnedValue.type === 'list') {
          const { type, limit, page, findCursor, transformer } = returnedValue;
          ...
          const [total, data] = await Promise.all([
            ...
            findCursor
              ...
              .limit(cLimit)
              .toArray()
              .then((arr) => arr.map(({ _id, ...data }) => data)),
          ]);
          ...
          return Promise.resolve(transformer ? transformer(data) : data)
            ...
            .then((data) => {
              return {
                type,
                total,
                data: data.map(({ _id, no, total_count, ...data }) => data),
              };
            });
        } else {
          throw new HttpException('Forbidden', HttpStatus.FORBIDDEN);
